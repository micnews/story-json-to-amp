...
const convertShadow = value => (Array.isArray(value) ? value : [value])
  .map(({ offset: { width, height }, color, radius }) => ([
    `${width}px`,
    `${height}px`,
    radius && `${radius}px`,
    color,
...
const convertCSS = (css) => {
  const convertedCSS = postcss(postCssPlugins).process(css, { parser: postcssJs }).css;

  // This fixes the omission of trailing semicolons, which seems to be a bug from postcss
  if (/\n}/.test(convertedCSS)) {
    return convertedCSS.replace(/\n}/g, ';\n}');
  }
