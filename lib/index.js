...
export default ({
  ...
  const renderAmpStoryGridLayer = ({ template, styles, id }, children) => renderTag(
  ...
  const layerTemplateToRenderMap = {
    fill: ({ id, element, styles }) => renderAmpStoryGridLayer(
      { template: 'fill', id, styles },
      element ? renderElement(element) : undefined,
    ),
    vertical: ({ id, elements, styles }) => renderAmpStoryGridLayer(
      { template: 'vertical', id, styles },
      elements && elements.map(element => renderElement(element)).join(''),
    ),
    horizontal: ({ id, elements, styles }) => renderAmpStoryGridLayer(
      { template: 'horizontal', id, styles },
      elements && elements.map(element => renderElement(element)).join(''),
    ),
    thirds: ({ id, elements, styles }) => renderAmpStoryGridLayer(
      { template: 'thirds', id, styles },
